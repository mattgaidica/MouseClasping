function cac = colorAutoCorrelogram(image)
...
cac = correlogram(rgb, map, distances);
cac = reshape(cac, [4 4 4]);

% consturct final correlogram using distances
cac(:, :, 1) = cac(:, :, 1)*distances(1);
cac(:, :, 2) = cac(:, :, 2)*distances(2);
cac(:, :, 3) = cac(:, :, 3)*distances(3);
cac(:, :, 4) = cac(:, :, 4)*distances(4);

% reform it to vector format
cac = reshape(cac, 1, 64);
